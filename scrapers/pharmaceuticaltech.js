/**
 * Pharmaceutical Technology Scraper
 * 
...
const SOURCE_NAME = 'PharmaceuticalTech';
const BASE_URL = 'https://www.pharmaceutical-technology.com';
...
export async function discoverUrlsFromSitemap() {
    ...
    try {
        ...
        if (sitemaps.length === 0) {
            ...
            return articles
                .filter(a => a.url && a.url.includes('pharmaceutical-technology.com'))
                .map(a => a.url);
        }
        ...
        for (let i = 0; i < sitemaps.length; i++) {
            ...
            try {
                ...
                const filtered = articles.filter(a => {
                    if (!a.url || !a.url.includes('pharmaceutical-technology.com')) {
                        return false;
                    }
                    // Filter out non-article pages
                    if (a.url.includes('/news/') || a.url.includes('/features/') || a.url.includes('/analysis/')) {
                        return true;
                    }
                    return false;
                });
    ...
    for (const rssUrl of rssUrls) {
        try {
            ...
            metadata.forEach(item => {
                if (item.link && item.link.includes('pharmaceutical-technology.com')) {
