 ...
 * Note: May have paywall - handle gracefully
 */
...
const SOURCE_NAME = 'STATNews';
const BASE_URL = 'https://www.statnews.com';
...
export async function discoverUrlsFromSitemap() {
    ...
    try {
        ...
        if (sitemaps.length === 0) {
            ...
            return articles
                .filter(a => a.url && a.url.includes('statnews.com'))
                .map(a => a.url);
        }
        ...
        for (let i = 0; i < sitemaps.length; i++) {
            ...
            try {
                ...
                const filtered = articles.filter(a => {
                    if (!a.url || !a.url.includes('statnews.com')) {
                        return false;
                    }
    ...
    for (const rssUrl of rssUrls) {
        try {
            ...
            metadata.forEach(item => {
                if (item.link && item.link.includes('statnews.com')) {
...
export async function scrapeArticle(url) {
    try {
        ...
        // Check for paywall
        if ($('.paywall, .subscription-required, [class*="paywall"]').length > 0) {
            console.warn(`[${SOURCE_NAME}] Paywall detected for ${url}, extracting available content`);
        }
        ...
        for (const selector of contentSelectors) {
            const el = $(selector).first();
            if (el.length) {
                el.find('script, style, nav, footer, .advertisement, .ad, .related-articles, .social-share, .author-box, .paywall, .subscription-required').remove();
