/**
 * Business Weekly Scraper
 * 
 * Source: https://www.businessweekly.co.uk/
 * 
 * Implementation:
 * - Historical scraping: RSS feeds, sitemaps
 * - Weekly updates: RSS feed + full content scraping
 * - Filter for pharma/biotech/healthcare content
 */
...
const SOURCE_NAME = 'Business Weekly';
const BASE_URL = 'https://www.businessweekly.co.uk';
...
export async function discoverUrlsFromSitemap() {
    ...
    try {
        ...
        if (sitemaps.length === 0) {
            ...
            return articles
                .filter(a => {
                    if (!a.url || !a.url.includes('businessweekly.co.uk')) {
                        return false;
                    }
                    // Filter for pharma/biotech/healthcare keywords
                    const text = `${a.url}`.toLowerCase();
                    const keywords = ['pharma', 'pharmaceutical', 'drug', 'biotech', 'biotechnology', 
                                    'fda', 'clinical trial', 'medicine', 'therapeutic', 'treatment',
                                    'healthcare', 'medical', 'biologics', 'vaccine', 'cancer', 'oncology'];
        ...
        for (let i = 0; i < sitemaps.length; i++) {
            ...
            try {
                ...
                const filtered = articles.filter(a => {
                    ...
                    // Also check if it's in news/tech-trail/trade-floor sections
                    if (a.url.includes('/news/') || a.url.includes('/tech-trail/') || a.url.includes('/trade-floor/')) {
                        return true;
                    }
                    return false;
                });
    ...
    for (const rssUrl of rssUrls) {
        try {
            ...
            metadata.forEach(item => {
                if (item.link && item.link.includes('businessweekly.co.uk')) {
                    // Filter for pharma/biotech/healthcare keywords
...
/**
 * Discover article URLs by scraping listing pages and API endpoints
 */
export async function discoverUrlsFromPages() {
    console.log(`[${SOURCE_NAME}] Discovering URLs from article listing pages and API...`);
    ...
    // Business Weekly uses SvelteKit with API endpoints - try API first
    const apiEndpoints = [
        `${BASE_URL}/api/posts?limit=100&category_name=news`,
        `${BASE_URL}/api/posts?limit=100&category_name=tech_trail`,
        `${BASE_URL}/api/posts?limit=100&category_name=trade_floor`
    ];
        ...
        try {
            ...
            if (response.data && response.data.posts) {
                response.data.posts.forEach(post => {
                    if (post.path) {
                        const fullUrl = `${BASE_URL}${post.path}`;
                        const text = `${post.title || ''} ${post.teaser || ''}`.toLowerCase();
                        if (keywords.some(kw => text.includes(kw))) {
                            allUrls.add(cleanUrl(fullUrl));
                        }
                    }
                });
                console.log(`[${SOURCE_NAME}] Found ${response.data.posts.length} posts from API: ${apiUrl}`);
            }
        } catch (error) {
            console.warn(`[${SOURCE_NAME}] Error fetching API ${apiUrl}: ${error.message}`);
        }
    }
    
    // Also try scraping HTML pages for embedded JSON data
    const listingPages = [
        `${BASE_URL}/news`,
        `${BASE_URL}/tech-trail`,
        `${BASE_URL}/trade-floor`
    ];
    ...
    for (const pageUrl of listingPages) {
        try {
            ...
            // Extract JSON data from script tags (SvelteKit embeds data)
            const jsonMatches = response.data.match(/data-sveltekit-fetched[^>]*data-url="([^"]+)"[^>]*>([^<]+)<\/script>/g);
            if (jsonMatches) {
            ...
            $('a[href*="/posts/"]').each((i, el) => {
                ...
                if (href) {
                    ...
                    if (fullUrl.includes('businessweekly.co.uk') && fullUrl.includes('/posts/')) {
...
export async function scrapeArticle(url) {
    try {
        await randomDelay(5000, 10000); // Longer delays
        ...
        // Extract title - Business Weekly specific
        let title = '';
        ...
        if (!title) {
            ...
            if (pageTitle && !pageTitle.includes('Business Weekly')) {
...
export async function scrapeHistorical(options = {}) {
    ...
    const { limiter } = await createRateLimiter(BASE_URL, {
        delayBetweenRequests: 8000, // Longer delays for Business Weekly
        delayJitter: 3000,
        maxConcurrent: 1,
        batchSize: 20, // Smaller batches
        pauseBetweenBatches: 120000 // 2 minute pause between batches
    });
    ...
    const pageUrls = sitemapUrls.length === 0 && rssUrls.length === 0 ? await discoverUrlsFromPages() : [];
