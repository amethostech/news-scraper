/**
 * Nature Biotechnology Scraper
 * 
 * Source: https://www.nature.com/nbt/
 * 
 * Implementation:
...
const SOURCE_NAME = 'NatureBiotech';
const BASE_URL = 'https://www.nature.com';
...
export async function discoverUrlsFromSitemap() {
    ...
    try {
        ...
        if (sitemaps.length === 0) {
            ...
            return articles
                .filter(a => a.url && a.url.includes('/nbt/'))
                .map(a => a.url);
        }
        ...
        for (let i = 0; i < sitemaps.length; i++) {
            ...
            try {
                ...
                const filtered = articles.filter(a => {
                    if (!a.url || !a.url.includes('/nbt/')) {
                        return false;
                    }
                    // Filter out non-article pages
                    if (a.url.includes('/articles/') || a.url.match(/\/nbt\/v\d+\/n\d+/)) {
                        return true;
                    }
                    return false;
                });
    ...
    for (const rssUrl of rssUrls) {
        try {
            ...
            metadata.forEach(item => {
                if (item.link && item.link.includes('/nbt/')) {
...
export async function scrapeArticle(url) {
    try {
        ...
        for (const selector of contentSelectors) {
            const el = $(selector).first();
            if (el.length) {
                el.find('script, style, nav, footer, .advertisement, .ad, .related-articles, .social-share, .author-box, .c-article-references').remove();
...
export async function run(options = {}) {
    console.log(`\n=== ${SOURCE_NAME} SCRAPER ===`);
    console.log(`Source: ${BASE_URL}/nbt/\n`);
