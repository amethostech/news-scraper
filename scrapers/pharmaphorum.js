...
const SOURCE_NAME = 'Pharmaphorum';
const BASE_URL = 'https://pharmaphorum.com';
...
export async function discoverUrlsFromSitemap() {
    ...
    try {
        ...
        if (sitemaps.length === 0) {
            ...
            return articles
                .filter(a => a.url && a.url.includes('pharmaphorum.com'))
                .map(a => a.url);
        }
        ...
        for (let i = 0; i < sitemaps.length; i++) {
            ...
            try {
                ...
                const filtered = articles.filter(a => {
                    if (!a.url || !a.url.includes('pharmaphorum.com')) {
                        return false;
                    }
                    // Filter out non-article pages
                    if (a.url.includes('/news/') || a.url.includes('/article/') || a.url.includes('/opinion/')) {
                        return true;
                    }
                    return false;
                });
    ...
    for (const rssUrl of rssUrls) {
        try {
            ...
            metadata.forEach(item => {
                if (item.link && item.link.includes('pharmaphorum.com')) {
